...
var Slots = (function () {
    ...
    _createClass(Slots, [{
        ...
        value: function set() {
            ...
            var _reducePathAndValue = this.reducePathAndValue(Slots.path(path), value);
            ...
            var applyRules = function applyRules() {
                ...
                if (isFunction(rule)) {
                    ...
                    var newContext = rule.call(_this.getContext(result), val && isImmutable(val) && val.toJS() || val);
                    if (isPromise(newContext)) {
                        log("RETURNED PROMISE. BEGIN NEW CONTEXT");
                        newContext.bind(_this.getContext(_this.state)); // out of callstack (e.g. transaction context)
                        newContext.then(function (data) {
                            doSave(data.getState());
                        });
                    } else {
            ...
            function doSave(newState) {
                if (optimistic && !(0, _immutable.is)(self.state, newState)) {
                    if (save) {
                        log("SAVE %s", insp(newState));
                        self.state = newState;
                        if (!self.promises.length) {
                            self.onPromisesAreMadeListeners.forEach(function (f) {
                                return f(self.state.toJS());
                            });
                        }
                        self.onChangeListeners.forEach(function (f) {
                            return f(self.state.toJS());
                        });
