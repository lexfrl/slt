...
var Branch = (function () {
    ...
    _createClass(Branch, [{
        ...
        value: function set() {
            ...
            this.path = path;
            this.value = value;
            this.findAsync(new _immutable.List(path), value);
                ...
                if (rule) {
                    (function () {
                        var deps = _this.getDeps(_path).map(function (dep) {
                            var dependency = (0, _utils.toJS)(state.getIn(_slots2["default"].makePath(dep)));
        ...
        key: "resolveAsync",
        value: function resolveAsync() {
            var _this2 = this;

            Object.keys(this.locks).forEach(function (key) {
                var lock = _this2.locks[key];
                lock.promise.then(function (result) {
                    _this2.set(lock.path, result);
                    delete _this2.locks[key];
                });
            });
        }
    }, {
        key: "findAsync",
        value: function findAsync(path, value) {
            var _this3 = this;

            if ((0, _utils.isPromise)(value)) {
                this.locks[path] = { path: path, promise: value };
            } else if ((0, _utils.isObject)(value)) {
                Object.keys(value).forEach(function (k) {
                    return _this3.findAsync(path.push(k), value[k]);
                });
            }
        }
    }, {
